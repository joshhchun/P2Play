...
async def shell(peer):
    ...
    while True:
        ...
        if message[0] == "put":
            ...
            e = time_ns()
            print(f"Put {song_name} by {artist_name} on network in {(e-s) / (10**9)}s")
        if message[0] == "get":
            song_name, artist_name = message[1], message[2]
...
def main():
    ...
    loop.run_until_complete(peer.listen(10_000, "student05.cse.nd.edu"))
    s = time_ns()
    loop.run_until_complete(peer.bootstrap([('student05.cse.nd.edu', 9177 + num - 2)]))
    e = time_ns()
