...
class KClosestNodes:
    def __init__(self, node: Node, capacity: int, alpha: int):
        self.node      = node
        self.heap      = []
        self.contacted = set()
        self.exclude   = set()
...
class Crawler:
    def __init__(self, protocol, target, nodes, k, alpha, method):
        ...
        self.k        = k
        self.alpha    = alpha
        self.closest  = KClosestNodes(target, nodes, alpha)
        self.target   = target
        self.method   = getattr(self.protocol, "find_" + method)
    ...
    async def lookup(self):
        ...
        for node in self.closest.nearest():
            ...
            id_future_mapping[node.id] = self.method(node, self.target)
    ...
    def _handle_found_values(self, found_values):
        """
